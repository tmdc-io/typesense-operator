...
a failure of at most `(N-1)/2` nodes without losing its quorum. If the available nodes go below this threshold then two things
are taking place:
...
In a Kubernetes environment, nodes are `Pods` which are rather volatile by nature and their lifetime is quite ephemeral and subjects 
to potential restarts, put the whole concept of raft protocol consensus under a tough spot. As we can read in the official 
